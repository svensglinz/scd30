...
int scd30_init() {
  if ((scd30_fd = open(I2C_PATH, O_RDWR)) < 0) {
    return -1;
  }
  if ((ioctl(scd30_fd, I2C_SLAVE, SCD30_ADDR)) < 0) {
    return -1;
  }
  return 0;
}

int scd30_start_continuous_measure(uint16_t pressure) {
...
int scd30_write_cmd(uint16_t cmd) {
...
int scd30_write_cmd_arg(uint16_t cmd, uint16_t arg) {
    // convert to network order
    uint16_t cmd_no = htons(cmd);
...
int scd30_read(void *buf, size_t size) {
...
int scd30_stop_continous_measure() {
 ...
 */
int scd30_get_data_ready_status() {
  uint8_t read_buff[3];
...
int scd30_read_measurement(struct scd30_data *buf) {
...
int scd30_get_temperature_offset(uint16_t *offset) {
    uint8_t buf[3];
...
int scd30_set_altitude_compensation(uint16_t offset) {
...
int scd30_get_altitude_compensation(uint16_t *offset) {
...
int scd30_set_forced_recalibration_value(uint16_t concentration) {
...
int scd30_get_forced_recalibration_value(uint16_t *concentration) {
...
int scd30_set_measurement_interval(uint16_t interval) {
...
int scd30_get_measurement_interval(uint16_t *interval) {
...
int scd30_set_automatic_self_calibration(uint16_t status) {
...
int scd30_get_automatic_self_calibration(uint16_t *status) {
